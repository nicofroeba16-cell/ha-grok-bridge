...
class FileContractTests(unittest.TestCase):
    ...
    def test_rejects_path_traversal(self) -> None:
        with self.assertRaises(ValueError):
            bridge.safe_rel("../../secrets.yaml")

    def test_rejects_unknown_action(self) -> None:
        with self.assertRaises(ValueError):
            bridge.validate_request({"action": "shell"}, {"main_branch": "main"})

    def test_requires_full_sha_for_deploy(self) -> None:
        with self.assertRaises(ValueError):
            bridge.validate_request(
                {"action": "deploy", "target_commit": "abc"},
                {"main_branch": "main"},
            )
    ...
    def test_accepts_valid_deploy_scope(self, mocked_git) -> None:
        ...
        self.assertEqual("deploy", action)
        self.assertEqual({"dashboards", "themes"}, scopes)
    ...
    def test_scope_must_be_a_list(self) -> None:
        with self.assertRaises(ValueError):
            bridge.validate_request(
                {"action": "deploy", "target_commit": "a" * 40, "scope": "themes"},
                {"main_branch": "main"},
            )
    ...
    def test_failed_request_is_recorded_and_not_repeated(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(bridge, "ensure_repo"), \
                 patch.object(bridge, "read_ref_file", return_value={"id": "req-1", "action": "deploy"}), \
                 patch.object(bridge, "validate_request", side_effect=ValueError("bad request")), \
                 patch.object(bridge, "publish_status") as publish, \
                 patch.object(bridge, "LAST_REQUEST", last_request):
                bridge.process({"control_branch": "bridge-control"})
                bridge.process({"control_branch": "bridge-control"})

            self.assertEqual("req-1\n", last_request.read_text())
            self.assertEqual(1, publish.call_count)
            self.assertFalse(publish.call_args.args[1]["ok"])
    ...
    def test_removes_obsolete_allowed_files_only_in_scope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (destination / "dashboards").mkdir(parents=True)
            (destination / "themes").mkdir(parents=True)
            (source / "dashboards").mkdir(parents=True)
            (source / "themes").mkdir(parents=True)
            (destination / "dashboards" / "old.yaml").write_text("old")
            (destination / "themes" / "keep.yaml").write_text("keep")
            (source / "dashboards" / "new.yaml").write_text("new")

            removed = bridge.remove_allowed_not_in_source(
                destination, source, {"dashboards"}
            )

            self.assertEqual(["dashboards/old.yaml"], removed)
            self.assertFalse((destination / "dashboards" / "old.yaml").exists())
            self.assertTrue((destination / "themes" / "keep.yaml").exists())
    ...
    def test_failed_rollback_restores_pre_rollback_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config.mkdir()
            backups.mkdir()
            (config / "configuration.yaml").write_text("current")

            target_backup = backups / "target"
            target_backup.mkdir()
            (target_backup / "configuration.yaml").write_text("old")
            ...
            last_good = root / "last_known_good.json"
            last_good.write_text(json.dumps(previous))

            with patch.object(bridge, "CONFIG", config), \
                 patch.object(bridge, "BACKUPS", backups), \
                 patch.object(bridge, "LAST_GOOD", last_good), \
                 patch.object(bridge, "subprocess") as subprocess_mock:
            ...
            safety_backups = [
                p for p in backups.iterdir() if p.is_dir() and p.name != "target"
            ]
