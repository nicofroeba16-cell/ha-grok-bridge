...
Git is used for approved configuration files. Runtime state/database files are
deliberately excluded from Git. Full Home Assistant backups are a separate
concern and are not created by this module.
"""
...
ALLOWED_SCOPES = {
    "core", "automations", "scripts", "scenes", "packages",
    "dashboards", "themes", "www", "go2rtc", "custom_components",
}
...
def load_options() -> dict[str, Any]:
    defaults = {
        "poll_interval": 60,
        "config_repo": "",
        "control_branch": "bridge-control",
        "snapshot_branch": "live-snapshot",
        "status_branch": "bridge-status",
        "main_branch": "main",
    }
    if OPTIONS.is_file():
    ...
    if not isinstance(defaults["config_repo"], str) or not defaults["config_repo"]:
...
def allowed_path(path: str) -> bool:
    ...
    if normalized in DENIED_EXACT or any(
    ...
    return (
        ...
        and parts[1] in ALLOWED_COMPONENTS
    )
...
def ensure_repo(options: dict[str, Any]) -> None:
    if not WORK.exists():
        git(["clone", "--no-checkout", options["config_repo"], str(WORK)])
...
def read_ref_file(ref: str, path: str) -> dict[str, Any] | None:
    result = subprocess.run(
        ["git", "show", f"origin/{ref}:{path}"], cwd=WORK,
...
def checkout_branch(branch: str) -> None:
    ...
    if exists:
        git(["checkout", "-B", branch, f"origin/{branch}"], cwd=WORK)
    else:
...
def copy_allowed(source: Path, destination: Path, scopes: set[str] | None = None) -> list[str]:
    ...
    for file in source.rglob("*"):
        ...
        if not allowed_path(rel) or (scopes is not None and scope_for(rel) not in scopes):
            continue
...
def publish_status(options: dict[str, Any], entry: dict[str, Any]) -> None:
    entry["timestamp"] = now()
    write_branch_file(
        options["status_branch"], STATUS_PATH, entry,
        f"bridge status: {entry.get('action', 'unknown')}",
    )


def snapshot(options: dict[str, Any]) -> dict[str, Any]:
    checkout_branch(options["snapshot_branch"])
    for candidate in list(WORK.iterdir()):
    ...
    metadata = {
        ...
        "files": copied,
        "core_version": (
            (CONFIG / ".HA_VERSION").read_text().strip()
            if (CONFIG / ".HA_VERSION").is_file() else None
        ),
        "timestamp": now(),
    }
    meta = WORK / META_PATH
    ...
    if subprocess.run(["git", "diff", "--cached", "--quiet"], cwd=WORK).returncode != 0:
        git([
            "-c", "user.name=ha-file-sync-bridge",
            "-c", "user.email=bridge@local",
            "commit", "-m", "live snapshot",
        ], cwd=WORK)
        git(["push", "origin", f"HEAD:{options['snapshot_branch']}"], cwd=WORK)
    return metadata


def validate_request(request: dict[str, Any], options: dict[str, Any]) -> tuple[str, set[str]]:
    action = request.get("action")
    if action not in {"snapshot", "validate", "deploy", "rollback", "status"}:
    ...
    if scopes and not scopes <= ALLOWED_SCOPES:
        raise ValueError("invalid deploy scope")
...
def materialize(sha: str) -> Path:
    ...
    git(["worktree", "add", "--detach", str(target), sha], cwd=WORK)
    return target
...
def validate_candidate(candidate: Path, scopes: set[str]) -> list[str]:
    ...
    if not files:
        raise ValueError("no allowed files in requested scope")
    for rel in files:
...
def make_backup(scopes: set[str], files: set[str]) -> Path:
    ...
    (backup / "manifest.json").write_text(
        json.dumps(
            {"files": sorted(files), "existing": existing, "scopes": sorted(scopes), "timestamp": now()},
...
def restore_backup(backup: Path, files: set[str]) -> list[str]:
    ...
    for rel in sorted(files):
        if not allowed_path(rel):
            raise ValueError(f"backup contains invalid path: {rel!r}")
        source = backup / rel
        target = CONFIG / rel
        if source.is_file():
            target.parent.mkdir(parents=True, exist_ok=True)
...
def deploy(sha: str, scopes: set[str]) -> dict[str, Any]:
    ...
    try:
        candidate_files = set(validate_candidate(candidate, scopes))
        current_files = allowed_files(CONFIG, scopes)
        backup_files = current_files | candidate_files
        backup = make_backup(scopes, backup_files)
        removed = remove_allowed_not_in_source(CONFIG, candidate, scopes)
        for rel in sorted(candidate_files):
            target = CONFIG / rel
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(candidate / rel, target)
        check = subprocess.run(
            ["ha", "core", "check"], capture_output=True, text=True, timeout=180
        )
        if check.returncode:
            restore_backup(backup, backup_files)
            raise RuntimeError(
                f"ha core check failed: {(check.stdout + check.stderr)[-1000:]}"
            )
        ...
        return {
            "action": "deploy", "ok": True, "commit": sha,
            "files": sorted(candidate_files), "removed": removed,
        }
    finally:
        subprocess.run(
...
def rollback() -> dict[str, Any]:
    if not LAST_GOOD.is_file():
        raise ValueError("no last-known-good deployment is available")
    previous = json.loads(LAST_GOOD.read_text())
    backup = Path(previous["backup"])
    files = previous.get("files", [])
    if not backup.is_dir() or not isinstance(files, list):
        raise ValueError("last-known-good backup is invalid")
    restore_files = set(files)
    current_files = allowed_files(CONFIG)
    safety_backup = make_backup(set(ALLOWED_SCOPES), current_files | restore_files)
    try:
        ...
        if check.returncode:
            restore_backup(safety_backup, current_files | restore_files)
            raise RuntimeError(
                "rollback validation failed; pre-rollback state restored: "
                f"{(check.stdout + check.stderr)[-1000:]}"
            )
        return {
            "action": "rollback", "ok": True,
            "commit": previous["commit"], "files": restored,
        }
    finally:
        pass
...
def process(options: dict[str, Any]) -> None:
    ensure_repo(options)
    request = read_ref_file(options["control_branch"], REQUEST_PATH)
    if not request:
        return
    ...
    try:
        ...
        if action == "snapshot":
            ...
            result = deploy(request["target_commit"], scopes)
        elif action == "validate":
            candidate = materialize(request["target_commit"])
            try:
                result = {"action": "validate", "ok": True, "files": validate_candidate(candidate, scopes)}
            finally:
                subprocess.run(
                    ["git", "worktree", "remove", "--force", str(candidate)],
                    cwd=WORK, capture_output=True, timeout=60,
            ...
            result = {
                ...
                "last_known_good": json.loads(LAST_GOOD.read_text()) if LAST_GOOD.exists() else None,
            }
    except Exception as exc:
        result = {"action": str(request.get("action", "unknown")), "ok": False, "error": str(exc)}
    result["request_id"] = request_id
    try:
        publish_status(options, result)
    finally:
        LAST_REQUEST.write_text(request_id + "\n")
