...
class SimulationHarnessTests(unittest.TestCase):
    ...
    def test_partial_empty_degraded_and_large_variants_are_stable(self):
        matrix = simulation_matrix()
        self.assertEqual(set(matrix), {"mixed", "partial", "degraded", "empty", "large"})
    ...
    def test_large_payload_counts_remain_truthful(self):
        payload = build_simulation(worker_count=180, event_count=900, wake_count=500, partial=True)
