...
The current bridge version is **1.0.9**.
...
5. The installed HA File Sync Bridge 1.0.9 detects a new remote commit and deploys permitted files to `/config` automatically when `deploy_on_remote_change` is enabled.
...
- explicit cleanup of previously tracked excluded/runtime paths
...
- the Home Assistant instance is running HA File Sync Bridge 1.0.9 (or newer), and
...
- Do not treat `.cloud`, `.storage`, `.ssh`, `.cache`, databases, logs, or credentials as normal project source files.
...
Before making changes that depend on bridge behavior, inspect the actual installed/source version. This document describes the 1.0.9 contract and should be updated whenever the deployment contract changes materially.
