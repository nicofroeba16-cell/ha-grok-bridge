...
SENSITIVE_NAMES = {
    "secrets.yaml", ".env", ".env.local", ".env.production", ".env.development",
    "credentials.json", "credentials.yaml", "token.json", "service-account.json",
    "ha-grok-bridge.passphrase", "ha-file-sync-bridge.passphrase",
}
SENSITIVE_SUFFIXES = DEFAULT_EXCLUDED_SUFFIXES
SECRET_PATTERNS = [
...
CONFIG_SECRET_RE = re.compile(
    r'''(?im)^\s*(?:api[_-]?key|access[_-]?token|client[_-]?secret|private[_-]?key|password|passwd|secret|token)\s*[:=]\s*["']([^"']{12,})["']\s*(?:#.*)?$'''
)
...
def save(p, v):
    ...
    t.write_text(json.dumps(v, indent=2, sort_keys=True), encoding="utf-8")
    t.replace(p)
...
def csv(v, f):
    return {x.strip() for x in v.split(",") if x.strip()} if isinstance(v, str) else set(f)


def excluded(n, c):
    return (
        n in csv(c.get("exclude_names"), DEFAULT_EXCLUDED_NAMES)
        or n in csv(c.get("exclude_dirs"), DEFAULT_EXCLUDED_DIRS)
        or any(n.endswith(x) for x in csv(c.get("exclude_suffixes"), DEFAULT_EXCLUDED_SUFFIXES))
    )


def ignored(p, c):
    return any(excluded(x, c) for x in p.parts)


def ignore(c):
    return lambda _d, n: {x for x in n if excluded(x, c)}
...
def git(a, cwd=WORK, check=True):
    ...
    if check and p.returncode:
        raise RuntimeError(
            f"git failed ({p.returncode}): "
            f"{(p.stderr or p.stdout).strip().splitlines()[-1] if (p.stderr or p.stdout).strip() else 'unknown'}"
        )
    return p
...
def snapshot(c):
    ...
    t = SNAPSHOTS / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")
    shutil.copytree(CONFIG, t, ignore=ignore(c))
    lim = max(0, int(c.get("max_snapshots", 10)))
    xs = sorted(p for p in SNAPSHOTS.iterdir() if p.is_dir())
...
def clear(c):
    for p in list(WORK.iterdir()):
        if p.name == ".git" or excluded(p.name, c):
            continue
...
def to_work(c):
    ...
    for s in CONFIG.iterdir():
        if excluded(s.name, c):
            continue
        d = WORK / s.name
        shutil.copytree(s, d, ignore=ignore(c)) if s.is_dir() else shutil.copy2(s, d)
        n += 1
    return n
...
def tracked_sensitive(c):
    ...
    for x in git(["ls-files"]).stdout.splitlines():
        ...
        if ignored(p, c):
            continue
        if any(q in SENSITIVE_NAMES or q.endswith(tuple(SENSITIVE_SUFFIXES)) for q in p.parts):
            out.append(x)
    return out
...
def secret_scan(c):
    ...
    for p in WORK.rglob("*"):
        ...
        if ignored(r, c):
            continue
        if p.name in SENSITIVE_NAMES or any(q.endswith(tuple(SENSITIVE_SUFFIXES)) for q in r.parts):
        ...
        try:
            ...
            continue
        if any(x.search(t) for x in SECRET_PATTERNS):
            out.append(r.as_posix())
            continue
        if p.suffix.lower() in {".yaml", ".yml", ".json", ".env", ".ini", ".conf", ".cfg", ".toml"}:
            for m in CONFIG_SECRET_RE.finditer(t):
                if m.group(1).strip().lower() not in {
                    "changeme", "change-me", "your-token", "your_password", "placeholder", "example", "null", "none"
                }:
                    out.append(r.as_posix())
                    break
...
def push(b, dry):
    git(["add", "-A"])
    n = git(["diff", "--cached", "--name-status"]).stdout.splitlines()
    log(f"changes: {len(n)}")
    if not n:
        return False
    if dry:
    ...
    git(["push", "origin", b])
    git(["fetch", "--prune", "origin"])
    if git(["rev-parse", f"origin/{b}"]).stdout.strip() != h:
...
def status(**v):
    x = load(STATUS, {"version": VERSION})
    x.update(v)
    x.update(version=VERSION, updated_at=now())
    save(STATUS, x)
...
def sync(c, forced=None):
    ...
    try:
        ...
        if mode == "git_to_ha" or (mode == "bidirectional" and rc and not lc):
            ...
            if ts:
                raise RuntimeError(f"SECURITY BLOCKED: tracked sensitive path: {ts[0]}")
            snapshot(c)
            log(f"/config prepared: {to_work(c)} items")
            f = secret_scan(c)
            if f:
                raise RuntimeError(f"SECRET SCAN BLOCKED: {len(f)} finding(s): {', '.join(f[:5])}")
            push(b, dry)
        h = git(["rev-parse", "HEAD"]).stdout.strip()
        save(STATE, {**state(), "last_sync_commit": h, "last_config_hash": treehash(CONFIG, c), "last_success": now()})
        status(state="idle", last_sync=h, error=None)
        log("sync complete")
...
def safe_config_path(raw_path, c, allow_excluded=False):
    if not isinstance(raw_path, str) or not raw_path.strip():
        raise RuntimeError("path is required")
    raw = raw_path.strip().replace("\\", "/")
    p = Path(raw)
    if p.is_absolute():
        raise RuntimeError("absolute paths are not allowed; use a path relative to /config")
    if any(part in {"", "."} for part in p.parts):
        raise RuntimeError("invalid path")
    if ".." in p.parts:
        raise RuntimeError("path traversal is not allowed")
    if not allow_excluded and ignored(p, c):
...
def write_file(body, c):
    ...
    if not path_value:
        ...
        path_value = f"{str(directory).strip('/\\')}/{filename}" if directory else filename
    target, rel = safe_config_path(path_value, c)
    if target.name in SENSITIVE_NAMES or any(part.endswith(tuple(SENSITIVE_SUFFIXES)) for part in rel.parts):
        raise RuntimeError("target is sensitive and cannot be written by the bridge")
    if target.exists() and target.is_dir():
        raise RuntimeError("target is a directory")

    encoding = str(body.get("encoding", "utf-8")).lower()
...
def browse(path_value, c):
    target, rel = safe_config_path(path_value or ".", c)
    ...
    return {"path": f"/config/{rel.as_posix()}" if rel.as_posix() != "." else "/config", "entries": entries}
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/snapshots":
            ...
            return
        if parsed.path in {"/files", "/browse"}:
            try:
    ...
    def do_POST(self):
        try:
            ...
            body = json.loads(self.rfile.read(l) or b"{}") if l else {}
            if self.path in {"/sync", "/sync/bidirectional", "/sync/ha_to_git", "/sync/git_to_ha"}:
                m = "ha_to_git" if self.path.endswith("ha_to_git") else ("git_to_ha" if self.path.endswith("git_to_ha") else None)
                threading.Thread(target=lambda: sync(cfg(), m), daemon=True).start()
                self.out(202, {"accepted": True})
                return
            if self.path == "/write":
                result = write_file(body, cfg())
                self.out(201, result)
                return
...
def main():
    ...
    def worker():
        ...
        while True:
            try:
                c = cfg()
                m = str(c.get("initial_sync", "ha_to_git")) if first and not state().get("last_sync_commit") else None
                sync(c, m)
            except Exception:
                pass
            first = False
