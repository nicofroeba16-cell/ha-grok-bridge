...
SECRET_PATTERNS = [
    ...
    re.compile(r"(?i)\b(api[_-]?key|access[_-]?token|client[_-]?secret|private[_-]?key)\s*[:=]"),
    re.compile(r"(?i)\b(password|passwd|token|secret)\s*[:=]\s*['\"][^'\"]+['\"]"),
]
...
def excluded(name: str, cfg: dict[str, Any]) -> bool:
    names = csv_set(cfg.get("exclude_names"), DEFAULT_EXCLUDED_NAMES)
    dirs = csv_set(cfg.get("exclude_dirs"), DEFAULT_EXCLUDED_DIRS)
    suffixes = csv_set(cfg.get("exclude_suffixes"), DEFAULT_EXCLUDED_SUFFIXES)
    return name in names or name in dirs or any(name.endswith(s) for s in suffixes)


def ignore_config(cfg: dict[str, Any]):
    def ignore(_directory: str, names: list[str]) -> set[str]:
        return {n for n in names if excluded(n, cfg)}
    return ignore
...
def run_git(args: list[str], cwd: Path = WORK, check: bool = True) -> subprocess.CompletedProcess[str]:
    ...
    if p.stdout.strip():
        log(f"git stdout: {p.stdout.strip()}")
    if p.stderr.strip():
...
def tree_hash(root: Path, cfg: dict[str, Any]) -> str:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if path.is_file():
            h.update(rel.encode()); h.update(b"\0"); h.update(path.read_bytes())
    return h.hexdigest()
...
def copy_config_to_work(cfg: dict[str, Any]) -> int:
    ...
    for src in CONFIG.iterdir():
        ...
        dst = WORK / src.name
        if src.is_dir(): shutil.copytree(src, dst, ignore=ignore_config(cfg))
...
def copy_work_to_config(cfg: dict[str, Any], dry_run: bool = False) -> int:
    if not CONFIG.is_dir(): raise RuntimeError("/config is not mapped")
    count = 0
    remote_names = {p.name for p in WORK.iterdir() if p.name != ".git"}
...
def save_state(**kwargs: Any) -> None:
    s = state(); s.update(kwargs); atomic_json(STATE, s)


def detect_conflict(cfg: dict[str, Any]) -> bool:
    s = state()
    last_commit = s.get("last_sync_commit")
    local_hash = s.get("last_config_hash")
    if not last_commit or not local_hash: return False
    remote_changed = run_git(["rev-parse", "HEAD"], check=False).stdout.strip() != last_commit
    local_changed = tree_hash(CONFIG, cfg) != local_hash
    return remote_changed and local_changed


def pull_remote(cfg: dict[str, Any], dry_run: bool) -> None:
    branch = str(cfg.get("branch", "main"))
    run_git(["fetch", "--prune", "origin"])
    run_git(["checkout", branch])
    run_git(["reset", "--hard", f"origin/{branch}"])
    if detect_conflict(cfg):
        raise RuntimeError("SYNC CONFLICT: /config and GitHub changed since last synchronization")
    snapshot(cfg)
    n = copy_work_to_config(cfg, dry_run)
    log(f"GitHub -> /config: {n} items prepared")
...
def sync_cycle(cfg: dict[str, Any], force_mode: str | None = None) -> None:
    ...
    mode = force_mode or str(cfg.get("sync_mode", "bidirectional"))
    ensure_repo(str(cfg["config_repo"]), branch)
    validate_repo()
    if mode == "git_to_ha":
        pull_remote(cfg, dry); save_state(last_sync_commit=run_git(["rev-parse", "HEAD"]).stdout.strip(), last_config_hash=tree_hash(CONFIG, cfg)); return
    if mode == "ha_to_git":
        snapshot(cfg); n = copy_config_to_work(cfg); log(f"/config sync: {n} top-level items prepared")
        findings = secret_scan(cfg)
        if findings: raise RuntimeError("SECRET SCAN BLOCKED: " + ", ".join(findings))
        commit_and_push(branch, dry)
        save_state(last_sync_commit=run_git(["rev-parse", "HEAD"]).stdout.strip(), last_config_hash=tree_hash(CONFIG, cfg)); return
    # bidirectional: first detect remote changes, then sync local changes.
    run_git(["fetch", "--prune", "origin"])
    local_changed = tree_hash(CONFIG, cfg) != state().get("last_config_hash")
    remote_changed = run_git(["rev-parse", "HEAD"], check=False).stdout.strip() != state().get("last_sync_commit")
    if remote_changed and local_changed and state().get("last_sync_commit"):
        raise RuntimeError("SYNC CONFLICT: both /config and GitHub changed")
    if remote_changed and not local_changed:
        run_git(["checkout", branch]); run_git(["reset", "--hard", f"origin/{branch}"])
        snapshot(cfg); copy_work_to_config(cfg, dry); log("GitHub -> /config synchronized")
    elif local_changed or not state().get("last_sync_commit"):
        snapshot(cfg); n = copy_config_to_work(cfg); log(f"/config sync: {n} top-level items prepared")
        findings = secret_scan(cfg)
        if findings: raise RuntimeError("SECRET SCAN BLOCKED: " + ", ".join(findings))
        commit_and_push(branch, dry)
...
def restore_snapshot(name: str, cfg: dict[str, Any], dry_run: bool = False) -> None:
    ...
    clear = [p for p in CONFIG.iterdir() if not excluded(p.name, cfg)]
    for p in clear:
        if not dry_run: shutil.rmtree(p) if p.is_dir() else p.unlink()
    if not dry_run:
        for p in src.iterdir():
            if excluded(p.name, cfg): continue
            dst = CONFIG / p.name
            shutil.copytree(p, dst, ignore=ignore_config(cfg)) if p.is_dir() else shutil.copy2(p, dst)
...
def history_cleanup(cfg: dict[str, Any]) -> None:
    ...
    paths = csv_set(cfg.get("exclude_names"), DEFAULT_EXCLUDED_NAMES) | csv_set(cfg.get("exclude_dirs"), DEFAULT_EXCLUDED_DIRS)
    paths |= {"*.passphrase", "*.pem", "*.key", "*.p12", "*.pfx"}
    index = "git rm -r --cached --ignore-unmatch " + " ".join(sorted(paths))
    run_git(["filter-branch", "--force", "--index-filter", index, "--prune-empty", "--tag-name-filter", "cat", "--", "--all"], check=True)
    run_git(["reflog", "expire", "--expire=now", "--all"])
    run_git(["gc", "--prune=now", "--aggressive"])
    run_git(["push", "--force", "--all", "origin"])
    run_git(["push", "--force", "--tags", "origin"], check=False)
    log("history cleanup completed")
...
def main() -> None:
    ...
    while True:
        started=time.time(); result="ok"; error=""
        try:
            cfg=options()
            with LOCK.open("x"):
                if not state().get("initialized"):
                    mode=str(cfg.get("initial_sync","ha_to_git")); sync_cycle(cfg,mode); save_state(initialized=True)
                else: sync_cycle(cfg)
            ...
            status={"version":VERSION,"timestamp":utc_now(),"result":result,"error":error,"repo":options().get("config_repo"),"branch":options().get("branch"),"config_hash":tree_hash(CONFIG,options()) if CONFIG.is_dir() else None,"last_sync_commit":state().get("last_sync_commit")}
